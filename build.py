...
# Base URL of the tailgate order API (overridable for local testing)
TAILGATE_API_BASE = os.environ.get(
    "TAILGATE_API_BASE", "https://order.germanbakingasheville.com"
).rstrip("/")
...
def emit_brand_artifacts(base_dir: Path) -> None:
    """Emit shared brand artifacts for other surfaces (order pages, newsletter).

    One source of truth: the site's own styles.css and templates. Consumers
    (tailgate order pages, newsletter landing) link or sync these instead of
    hand-copying tokens — drift becomes impossible by construction.
    """
    import re
    ...
    # tokens.css — the :root design-token block extracted from styles.css
    styles = (base_dir / "styles.css").read_text(encoding="utf-8")
    match = re.search(r":root\s*\{[^}]*\}", styles)
    if match:
        tokens = (
            "/* Generated by site/build.py — do not edit. Source: styles.css :root */\n"
            + match.group(0)
            + "\n"
        )
        (brand_dir / "tokens.css").write_text(tokens, encoding="utf-8")
        print(f"✓ brand/tokens.css generated ({len(tokens)} bytes)")

    # nav.html / footer.html — markup partials with a parameterized link base
    env = Environment(
        loader=FileSystemLoader(base_dir / "templates"),
        autoescape=select_autoescape(["html", "xml"]),
    )
    site_url = "https://germanbakingasheville.com"
    nav_html = env.get_template("partials/nav.html").render(nav_base=f"{site_url}/")
    (brand_dir / "nav.html").write_text(nav_html, encoding="utf-8")
    print("✓ brand/nav.html generated")

    footer_html = env.get_template("partials/footer.html").render()
    (brand_dir / "footer.html").write_text(footer_html, encoding="utf-8")
    print("✓ brand/footer.html generated")


def build_cart_page(env: Environment) -> None:
    """Generate /cart/ — the pre-order page (client-side, API-driven)."""
    template = env.get_template("cart.html")
    output_dir = Path(__file__).parent / "cart"
    output_dir.mkdir(exist_ok=True)
...
def build_all():
    ...
    # Shared brand artifacts for other surfaces (order pages, newsletter)
    emit_brand_artifacts(base_dir)
    ...
    # cache-busting version for the ordering script (mtime-based)
    env.globals["js_version"] = str(int((base_dir / "preorder.js").stat().st_mtime))
